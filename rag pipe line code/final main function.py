...
"""
def legal_rag_pipeline(file_path, user_question):
    global current_pdf_hash, final_summary

    incoming_pdf_hash = get_file_hash(file_path)
    is_new_pdf = incoming_pdf_hash != current_pdf_hash
    ...
    if is_new_pdf:
        ...
        unload_pegasus()
        mytho_tokenizer, mytho_model = load_mythomax()
        current_pdf_hash = incoming_pdf_hash
    else:
        if mytho_model is None:
            mytho_tokenizer, mytho_model = load_mythomax()
